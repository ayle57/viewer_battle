 ...
 * only what the server already sent, never invents anything. Steam
 * Ratings-specific shape (no `pointsAwarded` — every correct round is
 * worth exactly 1 point, see src/domain/game/steamRatings/types.ts's top
 * comment; `ROUND_CLOSED`, same event name as MusicEngine's own), so
 * this lives next to the rest of this game's view code rather than
 * reusing boardQuestion/events.ts's own describeLastResult.
 */
