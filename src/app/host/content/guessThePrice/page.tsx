...
const GAME_KEY = "guessThePrice" as const;

/**
 * Guess the Price's playlist library — the counterpart to
 * ../geoguessr/page.tsx / ../drawing/page.tsx / ../music/page.tsx /
 * ../steamRatings/page.tsx. Reads `content.pricePlaylist.*`
 * (contentPriceRouter.ts). A separate component, not a gameKey branch
 * inside any other page — same "don't touch the other games' own files"
 * posture every other Content Studio page in this app already follows.
 * No photo thumbnail strip on each card — same "keep the library card
 * text-only, save the visual detail for the editor" reasoning Steam
 * Ratings' own library page uses for its covers.
 *
 * Creation flow mirrors Jeopardy's own CreatePlaylistDialog (name +
 * optional description, entered up front).
 */
export default function GuessThePriceContentPage() {
  ...
  const playlists = trpc.content.pricePlaylist.list.useQuery({ token, gameKey: GAME_KEY }, { enabled: Boolean(token) });
  const createPlaylist = trpc.content.pricePlaylist.create.useMutation({
    onSuccess: (playlist) => {
      void utils.content.pricePlaylist.list.invalidate();
      router.push(`/host/content/guessThePrice/playlists/${playlist.id}`);
    },
  });
  const duplicatePlaylist = trpc.content.pricePlaylist.duplicate.useMutation({
    onSuccess: () => void utils.content.pricePlaylist.list.invalidate(),
  });
  const renamePlaylist = trpc.content.pricePlaylist.update.useMutation({
    onSuccess: () => {
      void utils.content.pricePlaylist.list.invalidate();
      setRenamingId(null);
    },
  ...
  return (
    <>
      <StudioBreadcrumb crumbs={[{ label: "Content Studio", href: "/host/content" }, { label: "Guess the Price" }]} />

      <div className={styles.header}>
        <div>
          <h1 className={styles.title}>Guess the Price</h1>
          <p className={styles.subtitle}>Prepare items, their photos, and their real prices.</p>
        </div>
      ...
      {playlists.data && playlists.data.length === 0 && (
        ...
        >
          ...
          <h2 className={styles.emptyTitle}>Build your first Guess the Price show.</h2>
          <p className={styles.emptySubtitle}>Add items, photos, and prices, and you&apos;re ready to play.</p>
          <Button size="lg" onClick={() => setCreateOpen(true)}>
            + Create Playlist
          </Button>
        </motion.div>
                ...
                >
                  <div className={styles.playlistCardTop}>
                    {renaming ? (
                      ...
                      <Link href={`/host/content/guessThePrice/playlists/${playlist.id}`} className={styles.playlistName}>
                        {playlist.name}
                      </Link>
                    )}
                  ...
                  <p className={styles.playlistMeta}>
                    {playlist.readiness.status === "incomplete"
                      ? `${playlist.readiness.completeItemCount} / ${playlist.readiness.itemCount} ready`
                      : `${playlist.itemCount} item${playlist.itemCount === 1 ? "" : "s"}`}
                  </p>
                  {playlist.readiness.status === "incomplete" && (
                    <div className={styles.cardProgressTrack} aria-hidden="true">
                      <div className={styles.cardProgressFill} style={{ width: `${(playlist.readiness.completeItemCount / playlist.readiness.itemCount) * 100}%` }} />
                    </div>
                  )}
                  ...
                  {!playlist.readiness.ready && playlist.readiness.status !== "empty" && (
                    <Link href={`/host/content/guessThePrice/playlists/${playlist.id}`} className={styles.reviewLink}>
                  ...
                  <div className={styles.playlistFooter}>
                    ...
                    <div className={styles.playlistActions}>
                      <Link href={`/host/content/guessThePrice/playlists/${playlist.id}`}>
      ...
      <ConfirmDialog
        ...
        description={
          pendingDeletePlaylist
            ? `"${pendingDeletePlaylist.name}" and its ${pendingDeletePlaylist.itemCount} item${pendingDeletePlaylist.itemCount === 1 ? "" : "s"} will be permanently removed.`
            : "This will permanently remove this playlist and its items."
        }
      ...
      <div className={styles.createLayout}>
        ...
        >
          <Input
            ...
            placeholder="e.g. Steam Sale Special"
            autoFocus
          />
          <Input
        ...
        <div className={styles.previewColumn}>
          ...
          <div className={styles.previewCard}>
            ...
            <p className={styles.playlistMeta}>0 items</p>
