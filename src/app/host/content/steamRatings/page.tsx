...
const GAME_KEY = "steamRatings" as const;

/**
 * Guess the Game's playlist library — the Steam Ratings counterpart to
 * ../geoguessr/page.tsx / ../drawing/page.tsx / ../music/page.tsx /
 * ../jeopardy/page.tsx. Reads `content.steamPlaylist.*`
 * (contentSteamRouter.ts). A separate component, not a gameKey branch
 ...
 * No cover-art thumbnail strip on each card — same "keep the library
 * card text-only, save the visual detail for the editor" reasoning
 * Music's own library page uses for its clips.
 *
 ...
 */
export default function SteamRatingsContentPage() {
  ...
  const playlists = trpc.content.steamPlaylist.list.useQuery({ token, gameKey: GAME_KEY }, { enabled: Boolean(token) });
  const createPlaylist = trpc.content.steamPlaylist.create.useMutation({
    onSuccess: (playlist) => {
      void utils.content.steamPlaylist.list.invalidate();
      router.push(`/host/content/steamRatings/playlists/${playlist.id}`);
    },
  });
  const duplicatePlaylist = trpc.content.steamPlaylist.duplicate.useMutation({
    onSuccess: () => void utils.content.steamPlaylist.list.invalidate(),
  });
  const renamePlaylist = trpc.content.steamPlaylist.update.useMutation({
    onSuccess: () => {
      void utils.content.steamPlaylist.list.invalidate();
      setRenamingId(null);
    },
  ...
  return (
    <>
      <StudioBreadcrumb crumbs={[{ label: "Content Studio", href: "/host/content" }, { label: "Guess the Game" }]} />

      <div className={styles.header}>
        <div>
          <h1 className={styles.title}>Guess the Game</h1>
          <p className={styles.subtitle}>Prepare Steam games and the reviews that give them away.</p>
        </div>
      ...
      {playlists.data && playlists.data.length === 0 && (
        ...
        >
          ...
          <h2 className={styles.emptyTitle}>Build your first Guess the Game show.</h2>
          <p className={styles.emptySubtitle}>Add games, cover art, and reviews, and you&apos;re ready to play.</p>
          <Button size="lg" onClick={() => setCreateOpen(true)}>
            + Create Playlist
          </Button>
        </motion.div>
                ...
                >
                  <div className={styles.playlistCardTop}>
                    {renaming ? (
                      ...
                      <Link href={`/host/content/steamRatings/playlists/${playlist.id}`} className={styles.playlistName}>
                        {playlist.name}
                      </Link>
                    )}
                  ...
                  <p className={styles.playlistMeta}>
                    {playlist.readiness.status === "incomplete"
                      ? `${playlist.readiness.completeGameCount} / ${playlist.readiness.gameCount} ready`
                      : `${playlist.gameCount} game${playlist.gameCount === 1 ? "" : "s"}`}
                  </p>
                  {playlist.readiness.status === "incomplete" && (
                    <div className={styles.cardProgressTrack} aria-hidden="true">
                      <div className={styles.cardProgressFill} style={{ width: `${(playlist.readiness.completeGameCount / playlist.readiness.gameCount) * 100}%` }} />
                    </div>
                  )}
                  ...
                  {!playlist.readiness.ready && playlist.readiness.status !== "empty" && (
                    <Link href={`/host/content/steamRatings/playlists/${playlist.id}`} className={styles.reviewLink}>
                  ...
                  <div className={styles.playlistFooter}>
                    ...
                    <div className={styles.playlistActions}>
                      <Link href={`/host/content/steamRatings/playlists/${playlist.id}`}>
      ...
      <ConfirmDialog
        ...
        description={
          pendingDeletePlaylist
            ? `"${pendingDeletePlaylist.name}" and its ${pendingDeletePlaylist.gameCount} game${pendingDeletePlaylist.gameCount === 1 ? "" : "s"} will be permanently removed.`
            : "This will permanently remove this playlist and its games."
        }
      ...
      <div className={styles.createLayout}>
        ...
        <div className={styles.previewColumn}>
          ...
          <div className={styles.previewCard}>
            ...
            <p className={styles.playlistMeta}>0 games</p>
