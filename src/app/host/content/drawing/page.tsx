...
const GAME_KEY = "drawing" as const;

/**
 * Drawing's playlist library — the Drawing counterpart to
 * ../geoguessr/page.tsx / ../jeopardy/page.tsx. Reads `content.
 * drawingPlaylist.*` (contentDrawingRouter.ts). A separate component, not
 * a gameKey branch inside either other page — same "don't touch the
 * other games' own files" posture as GeoGuessr's own page.
 *
 ...
 */
export default function DrawingContentPage() {
  ...
  const playlists = trpc.content.drawingPlaylist.list.useQuery({ token, gameKey: GAME_KEY }, { enabled: Boolean(token) });
  const createPlaylist = trpc.content.drawingPlaylist.create.useMutation({
    onSuccess: (playlist) => {
      void utils.content.drawingPlaylist.list.invalidate();
      router.push(`/host/content/drawing/playlists/${playlist.id}`);
    },
  });
  const duplicatePlaylist = trpc.content.drawingPlaylist.duplicate.useMutation({
    onSuccess: () => void utils.content.drawingPlaylist.list.invalidate(),
  });
  const renamePlaylist = trpc.content.drawingPlaylist.update.useMutation({
    onSuccess: () => {
      void utils.content.drawingPlaylist.list.invalidate();
      setRenamingId(null);
    },
  ...
  return (
    <>
      <StudioBreadcrumb crumbs={[{ label: "Content Studio", href: "/host/content" }, { label: "Drawing" }]} />

      <div className={styles.header}>
        <div>
          <h1 className={styles.title}>Drawing</h1>
          <p className={styles.subtitle}>Prepare the words your drawers will get, and how long they have.</p>
        </div>
      ...
      {playlists.data && playlists.data.length > 0 && <p className={styles.sectionLabel}>Your Prompt Lists</p>}
      ...
      {playlists.isError && (
        <div className={styles.errorState}>
          <p className={styles.emptyTitle}>Couldn&apos;t load your prompt lists.</p>
      ...
      {playlists.data && playlists.data.length === 0 && (
        ...
        >
          ...
          <p className={styles.emptyEyebrow}>First prompt list</p>
          <h2 className={styles.emptyTitle}>Build your first Drawing show.</h2>
          <p className={styles.emptySubtitle}>Add words to draw and set how long each one gets, and you&apos;re ready to play.</p>
          <Button size="lg" onClick={() => setCreateOpen(true)}>
            + Create Prompt List
          </Button>
        </motion.div>
                ...
                >
                  <div className={styles.playlistCardTop}>
                    {renaming ? (
                      <input
                        ...
                        aria-label="Rename prompt list"
                      />
                    ) : (
                      <Link href={`/host/content/drawing/playlists/${playlist.id}`} className={styles.playlistName}>
                        {playlist.name}
                      </Link>
                    )}
                  ...
                  <p className={styles.playlistMeta}>
                    {playlist.readiness.status === "incomplete"
                      ? `${playlist.readiness.completePromptCount} / ${playlist.readiness.promptCount} ready`
                      : `${playlist.promptCount} prompt${playlist.promptCount === 1 ? "" : "s"}`}
                  </p>
                  {playlist.readiness.status === "incomplete" && (
                    <div className={styles.cardProgressTrack} aria-hidden="true">
                      <div className={styles.cardProgressFill} style={{ width: `${(playlist.readiness.completePromptCount / playlist.readiness.promptCount) * 100}%` }} />
                    </div>
                  )}
                  ...
                  {!playlist.readiness.ready && playlist.readiness.status !== "empty" && (
                    <Link href={`/host/content/drawing/playlists/${playlist.id}`} className={styles.reviewLink}>
                  ...
                  <div className={styles.playlistFooter}>
                    ...
                    <div className={styles.playlistActions}>
                      <Link href={`/host/content/drawing/playlists/${playlist.id}`}>
      ...
      <ConfirmDialog
        ...
        description={
          pendingDeletePlaylist
            ? `"${pendingDeletePlaylist.name}" and its ${pendingDeletePlaylist.promptCount} prompt${pendingDeletePlaylist.promptCount === 1 ? "" : "s"} will be permanently removed.`
            : "This will permanently remove this prompt list and its prompts."
        }
...
function CreatePlaylistDialog({
  ...
  return (
    <Dialog open={open} onClose={handleClose} title="Create prompt list" description="A new prompt list, ready to fill in." size="lg">
      <div className={styles.createLayout}>
        ...
        >
          ...
          <Input
            ...
            placeholder="e.g. Community night prompt list"
          />
        ...
        <div className={styles.previewColumn}>
          ...
          <div className={styles.previewCard}>
            <div className={styles.playlistCardTop}>
              <span className={styles.playlistName}>{name.trim() || "Untitled Prompt List"}</span>
              <ReadinessBadge readiness={{ status: "empty" }} size="sm" />
            </div>
            <p className={styles.playlistMeta}>0 prompts</p>
