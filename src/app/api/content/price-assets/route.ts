...
import { MAX_UPLOAD_BYTES, saveUploadedPriceAsset } from "@/server/content/priceAssets";
import { logger } from "@/server/logger";

/**
 * The one write path for Guess the Price item photos ("le streamer
 * seulement" — gated behind a real ContentHost bearer token, the exact
 * same identity every other Content Studio mutation resolves, never a
 * public upload endpoint). A plain Next.js Route Handler, not tRPC —
 * same reasoning as src/app/api/content/steam-assets/route.ts's own doc
 * comment: tRPC's procedures are JSON-in/JSON-out and have no
 * multipart/binary story, while a Route Handler's `Request` is a real
 * Fetch API Request — `await request.formData()` parses
 * `multipart/form-data` natively.
 *
 * The actual save (validation + writing to disk) lives in
 * src/server/content/priceAssets.ts's `saveUploadedPriceAsset` — this
...
export async function POST(request: Request) {
  ...
  // reject it a moment later in saveUploadedPriceAsset's own size check.
  // Same posture as steam-assets/route.ts's own early check.
  ...
  try {
    const asset = await saveUploadedPriceAsset(file);
    return NextResponse.json(asset);
  } catch (error) {
    if (error instanceof ContentError) {
      return errorResponse(error.code, error.message, 400);
    }
    logger.error({ error }, "price item photo upload failed");
