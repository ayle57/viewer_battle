...
import { MAX_UPLOAD_BYTES, saveUploadedSteamAsset } from "@/server/content/steamAssets";
import { logger } from "@/server/logger";

/**
 * The one write path for Steam Ratings cover images ("le streamer
 ...
 * same reasoning as src/app/api/content/geo-assets/route.ts's /
 * music-assets/route.ts's own doc comment: tRPC's procedures are
 * JSON-in/JSON-out and have no multipart/binary story, while a Route
 * Handler's `Request` is a real Fetch API Request — `await
 * request.formData()` parses `multipart/form-data` natively.
 *
 * The actual save (validation + writing to disk) lives in
 * src/server/content/steamAssets.ts's `saveUploadedSteamAsset` — this
...
export async function POST(request: Request) {
  ...
  // reject it a moment later in saveUploadedSteamAsset's own size check.
  ...
  try {
    const asset = await saveUploadedSteamAsset(file);
    return NextResponse.json(asset);
  } catch (error) {
    if (error instanceof ContentError) {
      return errorResponse(error.code, error.message, 400);
    }
    logger.error({ error }, "steam cover asset upload failed");
